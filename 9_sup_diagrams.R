...
for (sp in usp) {
  ...
  k <- conexus::get_samplesize(adj_range, fun=fun) 
	zones <- try(conexus::make_zones(xy, k$range, k$n, spread=TRUE))
	if (inherits(zones, "try-error")) {
		zones <- conexus::make_zones(xy, k$range, k$n, spread=TRUE)
	}
...
# adjust to square to fill up the available space   
 ...
 par(mfrow=c(2,2), mar=rep(.5, 4))
  ...
  add_legend("bottomleft", legend=c("seed", "no seed"), title="Conservation\nzones", fill=c("light blue", "orange"), bty="o", bg="white")
  points(seed, pch=20, col="red", cex=1.5)
...
#  lines(network, col="red", lwd=network$weight/100)
  add_legend("bottomleft", legend=c("seed", "no seed"), title="Conservation\nzones", fill=c("light blue", "orange"), bty="o", bg="white")
  points(x, cex=2.5, col="white")
  text(x, cex=.8)
  legend("topleft", legend = "(C)", cex=1, bty="n")
   add_box()
  add_legend("bottomright", legend="Network", col="blue", lty=1, lwd=2, bg="white")
